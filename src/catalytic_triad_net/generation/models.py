...
# 🆕 导入 StoL 的高级几何编码
from .geometry_advanced import angle_emb, get_angle, get_distance
...
class E3EquivariantLayer(nn.Module):
    ...
    def __init__(self, hidden_dim: int, edge_dim: int = 0,
        ...
        if use_angle_encoding:
            ...
            logger.info(f"E3EquivariantLayer: 启用球谐函数角度编码 (维度: {angle_dim})")
        else:
            angle_dim = 0

        # 边缘MLP: 计算消息（🆕 增加角度特征维度）
        self.edge_mlp = nn.Sequential(
            nn.Linear(2 * hidden_dim + 1 + edge_dim + angle_dim, hidden_dim),
            act_fn,
    ...
    def forward(self, h: torch.Tensor, x: torch.Tensor,
        ...
        if self.use_angle_encoding:
            # 计算角度（简化版：使用三元组 i-j-k）
            angles = self._compute_angles(x, edge_index)  # [E]

            # 使用球谐函数编码角度
            angle_features = self.angle_encoder(
                dist.squeeze(-1),  # [E]
                angles             # [E]
            )  # [E, 128]
    ...
    def _compute_angles(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        """
        计算边的角度（简化版）

        对于每条边 (i, j)，计算它与相邻边的夹角
        这是一个简化实现，完整版需要构建三元组
        ...
        # 使用距离的归一化值作为伪角度（0 到 π）
        # 这是一个占位符，真正的实现需要构建 i-j-k 三元组
        angles = torch.clamp(dist / 10.0, 0, 1) * 3.14159
