...
# 常量定义
DEFAULT_TOP_N = 50  # 默认提取的功能团数量
DEFAULT_DEDUP_DISTANCE = 2.0  # Å - 去重距离阈值
MIN_ATOM_COMPLETENESS = 0.8  # 功能团原子完整性阈值（80%）
RCSB_PDB_URL_TEMPLATE = "https://files.rcsb.org/download/{pdb_id}.pdb"
CACHE_DIR_NAME = '.cache'
PROJECT_NAME = 'catalytic_triad_net'
PDB_SUBDIR = 'pdbs'
...
class FunctionalGroupExtractor:
    ...
    def extract_from_screening_results(self,
                                      ...
                                      top_n: int = DEFAULT_TOP_N) -> List[FunctionalGroup]:
        """
        从批量筛选结果中提取功能团
    ...
    def _extract_functional_group(self, structure, res_info: Dict,
        ...
        try:
            ...
            if len(coords) < len(template['atoms']) * MIN_ATOM_COMPLETENESS:
                logger.warning(f"功能团原子不完整: {resname}{resseq} "
                             f"({len(coords)}/{len(template['atoms'])} 原子)")
                return None
    ...
    def _download_pdb(self, pdb_id: str) -> Path:
        ...
        cache_dir = Path.home() / CACHE_DIR_NAME / PROJECT_NAME / PDB_SUBDIR
        ...
        if not pdb_path.exists():
            ...
            try:
                ...
                logger.info(f"已下载PDB: {pdb_id}")
            except Exception as e:
                logger.error(f"下载PDB失败 {pdb_id}: {e}")
    ...
    def deduplicate(self, functional_groups: List[FunctionalGroup],
                   distance_threshold: float = DEFAULT_DEDUP_DISTANCE) -> List[FunctionalGroup]:
        """
        去重：移除空间上非常接近的功能团
