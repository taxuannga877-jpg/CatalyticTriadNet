...
class SwissProtDataFetcher:
    ...
    def fetch_enzymes_by_ec_class(
        ...
        Args:
            ...
            limit: 最大条目数
            reviewed: 仅获取Swiss-Prot审核过的条目（推荐True）
            has_structure: 仅获取有PDB结构的条目（推荐True用于训练）
            has_active_site: 仅获取有活性位点标注的条目（推荐True）
            has_catalytic_activity: 仅获取有催化活性描述的条目（推荐True）
        ...
        params = {
            'query': query,
            'format': 'json',
            'size': min(limit * 2, 500),  # 多获取一些，因为后面要过滤
        ...
        while len(raw_entries) < limit * 2:
            ...
            try:
                ...
                logger.info(f"已获取 {len(raw_entries)} 原始条目...")
        ...
        logger.info(f"✓ 过滤后: {len(filtered_entries)}/{len(raw_entries)} 条目")

        # 保存缓存
        with open(cache_file, 'w') as f:
            json.dump(filtered_entries, f, indent=2)

        logger.info(f"✓ 获取完成: {len(filtered_entries)} 高质量条目")
